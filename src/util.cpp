...
const char * const BITCOIN_CONF_FILENAME = "tincoin.conf";
const char * const BITCOIN_PID_FILENAME = "tincoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "tincoin" is a composite category enabling all Tincoin-related debug output
            if(ptrCategory->count(string("tincoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TincoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TincoinCore
    // Mac: ~/Library/Application Support/TincoinCore
    // Unix: ~/.tincoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TincoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/TincoinCore";
#else
    // Unix
    return pathRet / ".tincoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty tincoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override tincoin.conf
