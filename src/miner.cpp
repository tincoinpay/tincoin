...
// ***TODO*** ScanHash is not yet used in Tincoin
//
...
{
    LogPrintf("TincoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tincoin-miner");
    ...
    try {
        ...
        while (true) {
            ...
            */
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("TincoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("TincoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("TincoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("TincoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}
