 ...
 *
 *   // network
 *   CTincoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.tincoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CTincoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CTincoinNetwork(UniValue objIn)
    {
...
// // can be under: TincoinNetwork
// //   -- signature requirements : Key1(User)
// class CTincoinNetworkVariable : public CGovernanceObject
// {
// private:
